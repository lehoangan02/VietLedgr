...
export default function Ledger() {
  ...
  const fetchLedgerData = async () => {
    ...
    try {
      ...
      // 1. Get Store ID (Matching your DB data)
      // We use the ID found in your database logs earlier
      const HARDCODED_STORE_ID = "1cd61ba6-4943-4cf3-97de-d9e8716b3ebf";
      ...
      console.log("Requesting ledger for:", HARDCODED_STORE_ID);
      ...
      if (!ledgerRes.ok) {
          // If 403, it means your current USER is not linked to this STORE ID
          if (ledgerRes.status === 403) throw new Error("Permission Denied: User not linked to Store " + HARDCODED_STORE_ID);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Real-time double-entry bookkeeping</p>
          </div>
          <button 
            onClick={fetchLedgerData}
            className="flex items-center gap-2 px-5 py-2.5 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition shadow-lg active:scale-95"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="text-3xl font-mono font-bold text-gray-900 mt-2">{totals.debit.toLocaleString()} <span className="text-lg font-sans font-medium text-gray-400">VND</span></div>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <span className="text-xs font-bold text-emerald-600 uppercase tracking-widest">Total Credits</span>
            <div className="text-3xl font-mono font-bold text-gray-900 mt-2">{totals.credit.toLocaleString()} <span className="text-lg font-sans font-medium text-gray-400">VND</span></div>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <span className="text-xs font-bold text-indigo-600 uppercase tracking-widest">Net Balance</span>
            <div className={`text-3xl font-mono font-bold mt-2 ${totals.debit - totals.credit >= 0 ? 'text-gray-900' : 'text-red-600'}`}>
                {(totals.debit - totals.credit).toLocaleString()} <span className="text-lg font-sans font-medium text-gray-400">VND</span>
            </div>
          </div>
        </div>
          ...
          <table className="min-w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">Account Type</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {entries.length > 0 ? entries.map((entry) => (
                <tr key={entry.entry_id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-5 text-sm text-gray-500 whitespace-nowrap">{new Date(entry.entry_date).toLocaleDateString()}</td>
                  <td className="px-6 py-5">
                    <span className={`px-2 py-1 text-[10px] font-black rounded uppercase tracking-tighter border ${
                        entry.account_type === 'ASSET' ? 'bg-blue-50 text-blue-700 border-blue-100' : 
                  ...
                  <td className="px-6 py-5 text-right font-mono text-sm text-blue-600 font-semibold">
                    {Number(entry.debit_amount) > 0 ? Number(entry.debit_amount).toLocaleString() : '—'}
                  </td>
                  <td className="px-6 py-5 text-right font-mono text-sm text-emerald-600 font-semibold">
