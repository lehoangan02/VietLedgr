...
export default function EditProductPage() {
   ...
   useEffect(() => {
      async function fetchProductAndBatch() {
         ...
         // Fetch all batches, find the one with product_id === productId
         try {
            const batchesRes = await fetch(`${FASTAPI_URL}/api/batches`)
            if (batchesRes.ok) {
               const batchesData = await batchesRes.json()
               // batchesData could be an array or {items: array}
               const batches = Array.isArray(batchesData) ? batchesData : (batchesData.items ?? [])
               const batch = batches.find((b: any) => String(b.product_id) === String(productId))
               if (batch) {
                  // Set quantity and price from batch if present
                  setQuantity(batch.stock ?? '')
                  setPrice(batch.sale_price ?? '')
   ...
   // Handler to update batch when quantity or price changes
   async function updateBatchField(field: "stock" | "sale_price", value: number | '') {
      if (!batchId) return;
      try {
         await fetch(`${FASTAPI_URL}/api/batches/${batchId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
                  ...
                  <div className="grid grid-cols-3 gap-4">
                     <div>
                        ...
                        <input type="number" value={quantity} onChange={(e) => {
                           ...
                           setQuantity(val);
                           updateBatchField("stock", val);
                        }} className="w-full mt-1 border rounded px-3 py-2" />
                     </div>
                     <div>
                        <label className="text-sm text-gray-700">Quantity Alert</label>
                        <input type="number" value={quantityAlert} onChange={(e) => {
                           const val = e.target.value === '' ? '' : Number(e.target.value);
                           setPrice(val);
                           updateBatchField("sale_price", val);
                        }} className="w-full mt-1 border rounded px-3 py-2" />
                     </div>
                     <div>
