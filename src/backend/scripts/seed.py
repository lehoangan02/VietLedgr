...
from app.models import Role, Store, TaxDetail, User
from app.models.base import RetailCategory
from app.models.ledger import GeneralLedgerEntry
from app.models.product import Product, ProductCategory
# ✅ import Supplier directly (avoid __init__ import issues)
...
def upsert_product_categories(
    session,
    store: Store,
    vat_standard: TaxDetail,
    vat_reduced: TaxDetail,
    no_tax: TaxDetail,
):
...
def add_products(session, store: Store, category_map: dict):
    ...
    products = [
        (
            ...
            RetailCategory.FOOD,
            "Rice & Grains",
            "Premium jasmine rice, 5kg bag",
        ),
        (
            "Rice 10kg",
            RetailCategory.FOOD,
            "Rice & Grains",
            "Premium jasmine rice, 10kg bag",
        ),
        (
            "Glutinous Rice 2kg",
            RetailCategory.FOOD,
            "Rice & Grains",
            "Sticky rice for desserts, 2kg",
        ),
        (
            "Brown Rice 5kg",
            RetailCategory.FOOD,
        ...
        (
            ...
            RetailCategory.FOOD,
            "Instant Foods",
            "Popular instant noodles, chicken flavor",
        ),
        (
            "Cup Noodles",
            RetailCategory.FOOD,
            "Instant Foods",
            "Instant cup noodles, beef flavor",
        ),
        (
            "Instant Pho",
            RetailCategory.FOOD,
            "Instant Foods",
            "Vietnamese pho instant noodles",
        ),
        (
            "Ramen Bowl",
            RetailCategory.FOOD,
        ...
        (
            ...
            RetailCategory.FOOD,
            "Snacks",
            "Crispy potato chips, original flavor",
        ),
        ...
        (
            "Electric Kettle",
            RetailCategory.HOUSEHOLD,
            "Kitchen Appliances",
            "1.7L electric kettle with auto shut-off",
        ),
        (
            "Rice Cooker",
            RetailCategory.HOUSEHOLD,
            "Kitchen Appliances",
            "Multi-function rice cooker 1.8L",
        ),
        (
            "Blender",
            RetailCategory.HOUSEHOLD,
            "Kitchen Appliances",
            "High-speed blender 500W",
        ),
        (
            "Non-stick Pan",
            RetailCategory.HOUSEHOLD,
        ...
        (
            ...
            RetailCategory.HOUSEHOLD,
            "Personal Care",
            "Refreshing body wash gel",
        ),
        (
            "Toothpaste",
            RetailCategory.HOUSEHOLD,
            "Personal Care",
            "Whitening toothpaste 100g",
        ),
        (
            "Hand Soap",
            RetailCategory.HOUSEHOLD,
        ...
        (
            ...
            "Office Supplies",
            "200-page lined notebook A5",
        ),
        (
            "Notebook A4",
            RetailCategory.STATIONERY,
            "Office Supplies",
            "100-page ruled notebook A4",
        ),
        (
            "Ballpoint Pen",
            RetailCategory.STATIONERY,
        ...
        (
            ...
            RetailCategory.STATIONERY,
            "Office Supplies",
            "White erasers 3-pack",
        ),
        (
            "Highlighter Set",
            RetailCategory.STATIONERY,
            "Office Supplies",
            "Fluorescent highlighters 4-pack",
        ),
        (
            "Sticky Notes",
            RetailCategory.STATIONERY,
            "Office Supplies",
            "Colorful sticky note pads",
        ),
        (
            "Reusable Bag",
        ...
        (
            ...
            RetailCategory.OTHERS,
            "Miscellaneous",
            "Alkaline AA batteries pack of 4",
        ),
        (
            "Phone Cable",
            RetailCategory.OTHERS,
    ...
    for name, location in warehouses:
        wh = session.execute(
            select(Warehouse).where(
                Warehouse.name == name, Warehouse.store_id == store.id
            )
        ).scalar_one_or_none()
...
def upsert_suppliers_from_batch_data(session, batch_data):
    """
    Ensure suppliers exist for every supplier_name in batch_data.
    Returns dict: {supplier_name: supplier_id}
    """
    supplier_names = sorted({row[5] for row in batch_data if row[5]})
    if not supplier_names:
        return {}

    existing = (
        session.execute(select(Supplier).where(Supplier.name.in_(supplier_names)))
        .scalars()
        .all()
    )

    name_to_supplier = {s.name: s for s in existing}

    for name in supplier_names:
        if name not in name_to_supplier:
            s = Supplier(name=name)
            session.add(s)
            session.flush()  # populate s.id
            name_to_supplier[name] = s

    return {name: s.id for name, s in name_to_supplier.items()}
...
def add_batches(session, products: list[Product], warehouses: list[Warehouse]):
    ...
    batch_data = [
        # (sku, wh_idx, stock, cost, sale_price, supplier_name, days_until_expire)
        (
            "RICE_5KG",
            0,
            150,
    ...
    # ✅ Ensure suppliers exist and map name -> id
    supplier_map = upsert_suppliers_from_batch_data(session, batch_data)

    product_map = {p.sku: p for p in products}
    now = datetime.now()

    for sku, wh_idx, stock, cost, sale_price, supplier_name, expire_days in batch_data:
        ...
        # ✅ check duplicates using supplier_id (not supplier_name)
        ...
        import_days_ago = hash(sku + (supplier_name or "")) % 30 + 1
...
def seed_ledger_entries(session):
    ...
    existing = session.execute(
        ...
        .filter(GeneralLedgerEntry.store_id == store.id)
        .limit(1)
    ).scalar_one_or_none()
...
def main():
    ...
    with SessionLocal() as session:
        ...
        vat_standard = upsert_tax(
            session, "VAT Standard", Decimal("10.00"), "Standard VAT rate"
        )
        vat_reduced = upsert_tax(
            session, "VAT Reduced", Decimal("5.00"), "Reduced VAT rate"
        )
        no_tax = upsert_tax(session, "No Tax", Decimal("0.00"), "Tax exempt items")

        upsert_admin_user(session, store, admin_role)

        category_map = upsert_product_categories(
            session, store, vat_standard, vat_reduced, no_tax
        )
